...
define('JOOMLA_BIRTH_DAY', 1126742400 );

jimport( 'joomla.application.component.model' );

class JvcounterModelJvcounter extends JModelLegacy
{
    /**
    ...
    public static function getVisitsFromLogs($startTime=0, $stopTime=0){
        ...
        if ( $startTime < JOOMLA_BIRTH_DAY ) {	$startTime 	= 0; }

        if ( $stopTime < JOOMLA_BIRTH_DAY ) { $stopTime 	= 0;}

        $query = "SELECT * FROM " . $db->quoteName('#__jvcounter');
        $query .= " WHERE '1=1'";
    ...
    static function insertLog( $time, $visitsArray ){
        ...
        $query	= "INSERT INTO #__jvcounter (time, visits, guests, members, bots)";
    ...
    static function removeNullLogs(){
        ...
        $query = "SELECT ".$db->quoteName('time'). " FROM ". $db->quoteName('#__jvcounter')." WHERE ";
        ...
        if(count($times)){
            foreach($times as $time){
                $query = "DELETE FROM ".$db->quoteName('#__jvcounter')." WHERE ".$db->quoteName('time'). "=$time";
                $db->setQuery($query);
                $db->execute();
            }
            return;
    ...
    static function lastTimeLog() {
        ...
        $query = "SELECT MAX(". $db->quoteName('time').") FROM ". $db->quoteName('#__jvcounter');
        $db->setQuery( $query );
